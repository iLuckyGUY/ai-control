...
// macOS-specific tool schemas
export const AppleScriptArgsSchema = z.object({
  script: z.string().describe('AppleScript code to execute'),
});

export const DockerCommandArgsSchema = z.object({
  command: z.string().describe('Docker command, e.g. "ps -a", "restart nginx", "compose up -d"'),
  cwd: z.string().optional().describe('Working directory for docker compose'),
});
