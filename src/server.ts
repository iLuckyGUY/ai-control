...
const CMD_PREFIX_DESCRIPTION = `This command is part of AI Control MCP server.`;
...
import {
    ...
    WritePdfArgsSchema,
    AppleScriptArgsSchema,
    DockerCommandArgsSchema,
    SystemInfoArgsSchema,
} from './tools/schemas.js';
import { getConfig, setConfigValue } from './tools/config.js';
import { getUsageStats } from './tools/usage.js';
import { giveFeedback } from './tools/feedback.js';
...
function shouldIncludeTool(toolName: string): boolean {
    if (currentClient?.name === 'ai-control-app') {
        if (toolName === 'give_feedback' || toolName === 'get_prompts') {
            return false;
        }
    }
    ...
    try {
        ...
        const allTools = [
            ...
            },
            {
                name: "give_feedback",
                description: `
                        Open feedback form in browser to provide feedback about AI Control.
                        ...
                        3. Tool opens form with only usage statistics pre-filled automatically:
                           ...
                           - days_using: How many days they've used AI Control
                        ...
                        - AI Control's biggest advantage
            ...
            // ── macOS-specific tools ──
            {
                name: "applescript",
                description: `
                        Run AppleScript for GUI automation on macOS.
                        Can open applications, control system dialogs, automate Finder,
                        send notifications, and control any scriptable macOS app.

                        Examples:
                        - Tell Finder to open a folder
                        - Display a dialog to the user
                        - Control System Events for accessibility automation

                        ${CMD_PREFIX_DESCRIPTION}`,
                inputSchema: zodToJsonSchema(AppleScriptArgsSchema),
                annotations: {
            ...
            },
            {
                name: "docker_command",
                description: `
                        Execute Docker or Docker Compose commands directly.
                        Automatically prepends 'docker' if not specified.
                        ...
                        Works with docker compose by providing the cwd parameter
                        (working directory where docker-compose.yml is located).

                        ${CMD_PREFIX_DESCRIPTION}`,
                inputSchema: zodToJsonSchema(DockerCommandArgsSchema),
                annotations: {
            ...
            },
            {
                name: "system_info",
                description: `
                        Get real-time macOS system information: CPU model, uptime,
                        memory statistics (via vm_stat), and disk usage.

                        Useful for checking system health, available resources,
                        and diagnosing performance issues.

                        ${CMD_PREFIX_DESCRIPTION}`,
                inputSchema: zodToJsonSchema(SystemInfoArgsSchema),
                annotations: {
...

            {
                name: "get_prompts",
                description: `
                        Retrieve a specific AI Control onboarding prompt by ID and execute it.
...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest): Promise<ServerResult> => {
    ...
    try {
        ...
        switch (name) {
            ...
            case "give_feedback":
                try {
                    result = await giveFeedback(args);
                } catch (error) {
                    capture('server_request_error', { message: `Error in give_feedback handler: ${error}` });
                    result = {
            ...
            // macOS-specific tools
            case "applescript":
                result = await handlers.handleAppleScript(args);
                break;

            case "docker_command":
                result = await handlers.handleDockerCommand(args);
                break;

            case "system_info":
                result = await handlers.handleSystemInfo();
                break;
